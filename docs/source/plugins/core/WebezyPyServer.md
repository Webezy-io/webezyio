...
`WebezyPyServer` is a core plugin for building `Python` server from `webezyio` resources configured on `webezy.json` file.

## Overview

`WebezyPyServer` will be registered on `Builder` plugin-manager once it detects that the project hold a server resource of type `Python`:
...
On `wz build` process `WebezyPyServer` plugin will build the following files:
...
Mini hooks can be used on `WebezyPyServer` to inject the core plugin with additional parameters combined from different `Custom Plugins` that implementing:

- `pre_build_server`:
Hook implemantation for injecting data into `write_server` hook execution.

To write a custom plugin that uses `WebezyPyServer` see more information on [Writing Custom Plugin](../custom_plugins.md)

Main hook: `webezyio.builder.plugins.WebezyPyServer`
