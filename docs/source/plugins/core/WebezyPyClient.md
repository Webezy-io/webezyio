...
`WebezyPyClient` is a core plugin for building `Python` client from `webezyio` resources configured on `webezy.json` file.

## Overview

`WebezyPyClient` will be registered on `Builder` plugin-manager once it detects that the project hold a server resource of type `python`:

Start a `webezyio` project and pass in the `python` config:
...
On `wz build` process `WebezyPyClient` plugin will build the following files:
...
Mini hooks can be used on `WebezyPyClient` to inject the core plugin with additional parameters combined from different `Custom Plugins` that implementing:

- `pre_build_clients`:
Hook implemantation for injecting data into `write_clients` hook execution.

To write a custom plugin that uses `WebezyPyClient` see more information on [Writing Custom Plugin](../custom_plugins.md)

Main hook: `webezyio.builder.plugins.WebezyPyClient`
