...
class WebezyBuilder:
    ...
    def _parse_webezy_context(self, path):
        ...
        try:
            ...
            results= self._pm.hook.init_context(wz_json=self._webezy_json,wz_context=None)
            ...
            try:
                ...
                logging.error("Error init context, make sure you import a plugin that implement the `init_context` hook")


    def InitProjectStructure(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.init_project_structure` hook"""
        results = self._pm.hook.init_project_structure(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results
    ...
    def BuildProtos(self):
        ...
        results = self._pm.hook.write_protos(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results

    def CompileProtos(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.compile_protos` hook"""
        results = self._pm.hook.compile_protos(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results

    def BuildClients(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.write_clients` hook"""
        results = self._pm.hook.write_clients(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results

    def BuildServer(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.write_server` hook"""
        results = self._pm.hook.write_server(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results

    def WriteReadme(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.write_readme` hook"""
        results = self._pm.hook.write_readme(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results
    ...
    def OverrideGeneratedClasses(self):
        ...
        results = self._pm.hook.override_generated_classes(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results

    def PreBuild(self):
        """Executing the :func:`webezyio.builder.src.hookspecs.pre_build` hook"""
        results = self._pm.hook.pre_build(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results
    ...
    def PostBuild(self):
        ...
        results = self._pm.hook.post_build(wz_json=self._webezy_json,wz_context=self._webezy_context)
        results = list(itertools.chain(*results))
        return results
    ...
    def ParseProtosToResource(self,protos_dir=None,project_name=None,server_language=None,clients=[]):
        ...
        path = self._path.replace('webezy.json','protos') if protos_dir is None else protos_dir
        logging.info(path)
        ...
        results = self._pm.hook.parse_protos_to_resource(protos_dir=path,project_name=project_name,server_language=server_language,clients=clients)
        results = list(itertools.chain(*results))
        return results
    ...
    def BuildAll(self):
        ...
        results = [prebuild,init,context,protos,services,server,compile,readme,protoclass,clients,postbuild]
        return results
