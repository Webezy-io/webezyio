...
def parse_protos_to_resource(protos_dir,project_name,server_language,clients:List[int]=[Language.PYTHON]):
    ...
    temp_clients = []
    for c in clients:
        temp_clients.append(WebezyClient(language=Language.Name(c),out_dir=file_system.join_path(path,'clients',Language.Name(c).lower())))
    srvr = WebezyServer(language=Language.Name(Language.PYTHON))
    prj = Project(uri=path,name=project_name,package_name=project_name.replace('-','').replace('_',''),version='0.0.1',type=resources.ResourceTypes.project.name,kind=resources.ResourceKinds.ezy_1.name,server=srvr,clients=temp_clients)
    resources_added.append(WzResourceWrapper(project=prj))

    if file_system.walkFiles(protos_dir) is None:
        raise errors.WebezyProtoError("File","Error on parsing proto files. make sure you have a protos dir under current location.")
    ...
    for f in file_system.walkFiles(protos_dir):
        ...
        try:
            ...
            logging.info(f'[{protos.name}] {protos.package} | Has Opt: {protos.has_options} | {protos.extensions_by_name}')
            
            if protos.services_by_name != {} and protos.message_types_by_name != {}:
                raise errors.WebezyProtoError('Service','Service canot hold messages ! Please de-attach the messages to stand-alone package')
            ...
            if protos.package is not None and protos.package != '':
                ...
                pkgs[pkg_path] = { 'uri': resources.get_uri_package(path,protos.package), 'name': pkg_name, 'package':protos.package, 'messages':[],'enums':[]}
                for msg in protos.message_types_by_name:
                    ...
                    for f in desc.field:
                        ...
                        field = {'uri':resources.get_uri_field(path,field_full_name),'name':f.name,'fullName':field_full_name,'index':f.number,'fieldType': resources.FieldDescriptorProto.Type.Name(f.type),'label':resources.FieldDescriptorProto.Label.Name(f.label), 'type':resources.ResourceTypes.descriptor.value,'kind':resources.ResourceKinds.field.value}
                    ...
                    msg_dict = {'uri':resources.get_uri_message(path,msg_full_name),'name':desc.name,'fullName':msg_full_name, 'fields': fields}
                    
                    pkgs[pkg_path]['messages'].append(msg_dict)
                    resources_added.append(WzResourceWrapper(message=resources.ParseDict(msg_dict,Descriptor())))
            ...
            for svc in protos.services_by_name:
                ...
                for rpc in protos.services_by_name[svc].methods_by_name:
                    ...
                    rpcs.append({'uri':resources.get_uri_rpc(path,full_name),'name':rpc.name,'fullName':full_name,'outputType':output_type,'inputType':input_type,'clientStreaming':client_stream,'serverStreaming':server_stream})
                service = {'uri': resources.get_uri_service(path,svc,server_language), 'name':svc, 'methods': rpcs}
                services[svc] = service
                logging.info(f"Adding {svc}")
                resources_added.append(WzResourceWrapper(service=resources.ParseDict(service,ServiceDescriptor())))


        except Exception as e:
            logging.exception(f'Exception occured during handling of [{f}] : {e}')
