                ...
                if dict(self.__dict__)[k] is not None:
                    ...
                    for j in dict(self.__dict__)[k]:
                        if isinstance(dict(self.__dict__)[k][j],str):
                            temp[k[1:]][j] = Value(string_value=dict(self.__dict__)[k][j])
                        elif isinstance(dict(self.__dict__)[k][j],int):
                            temp[k[1:]][j] = Value(number_value=dict(self.__dict__)[k][j])
...
class WZMessage():
    ...
    def to_tuple(self):
        if self._fields is None:
            raise WebezyValidationError("Message","Message must hold atleast 1 field !")
...
class WZProto():
    ...
    def write_imports(self):
        if self._imports is not None:
            ...
            options = next((m for m in self._messages if m.get('extensionType') is not None),None)
            if options is not None:
                temp_imports.append('import "google/protobuf/descriptor.proto";')
            return '\n'.join(temp_imports)
        else:
            return ''
                ...
                for f in m.get('fields'):
                    ...
                    if f.get('extensions') is not None:
                        for ext in f.get('extensions'):
                            ...
                            if len(list_names) > 2:
                                ext_pkg = next((pkg for pkg in self._package if pkg == 'protos/{0}/{1}.proto'.format(ext.split('.')[2],ext.split('.')[1])))
                                if ext_pkg is not None:
                                    ext_msg = next((m for m in self._messages[ext_pkg] if m.get('name') == ext.split('.')[3]),None)
                                    logging.debug(f'{list_names} | {ext_msg}')
                            else:
                                ext_msg = next((m for m in self._messages if m.get('name') == ext.split('.')[0]),None)
                                logging.debug(f'{list_names} | {ext_msg}')
                            if ext_msg is None:
                                raise WebezyValidationError('FieldOptions',f'Field Option [{ext}] specified for : "{fName}", is invalid !')
                            field = next((f for f in ext_msg.get('fields') if f.get('name') == list_names[-1]),None) 
                            if field is None:
                                logging.error(f"Cant parse extension field [{f.get('fullName')}] {ext}")
                            else:
                                ext_v = f.get('extensions')[ext]
                                type_ext = field.get('fieldType').split('_')[-1].lower()
                                if 'int' in type_ext:
                                    ext_v = int(ext_v)
...
class WZClientPy():
    ...
    def write_imports(self):
        imports = ['from typing import Tuple,Iterator','import grpc','import sys','from functools import partial']
        for svc in self._services:
        ...
        if self._services is not None:
            ...
            for svc in self._services:
                for rpc in self._services[svc]['methods']:
                    ...
                    in_open_type = 'Iterator[' if rpc.get('clientStreaming') is not None and rpc.get('clientStreaming') == True else ''
                    in_close_type = ']' if rpc.get('clientStreaming') is not None and rpc.get('clientStreaming') == True else ''
                    out_open_type = 'Iterator[' if rpc.get('serverStreaming') is not None and rpc.get('serverStreaming') == True else ''
                    out_close_type = ']' if rpc.get('serverStreaming') is not None and rpc.get('serverStreaming') == True else ''
                    rpcs.append(f'def {rpc_name}(self, request: {in_open_type}{rpc_in_type}{in_close_type}, metadata: Tuple[Tuple[str,str]]) -> {out_open_type}{rpc_out_type}{out_close_type}:\n\t\t"""webezyio"""\n\n\t\treturn self.{svc}Stub.{rpc_name}(request,metadata=metadata)')
